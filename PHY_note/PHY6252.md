...
## BLE Mesh 的核心概念
BLE Mesh（Bluetooth Low Energy Mesh）是一种基于低功耗蓝牙（Bluetooth Low Energy, BLE）的网状网络通信协议。它允许多个支持 BLE 的设备组成一个分布式网络，通过设备之间的相互中继，实现大范围的、可靠的设备间通信。  
...
中继节点（Relay Node）：用于转发消息的节点，扩展网络范围。  
网关（Gateway）：连接 BLE Mesh 网络与其他网络（如 Wi-Fi、以太网）设备。  
代理节点（Proxy Node）：允许传统 BLE 设备（不支持 Mesh 的设备）与 Mesh 网络通信。  

代理节点、低功耗节点、中继节点、朋友节点、普通节点
