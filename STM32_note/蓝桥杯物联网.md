...
![alt text](image.png)
# 板卡介绍
产品基于 STM32WLE5CCU6 Sub-GHz 无线微控制器设计，芯片提供了 256KB FLASH 和 64KB SRAM 存储器，最高主频 48MHz，内部集成 ARM Cortex M4 内核和无线通讯单元，支持LoRa ®、(G)FSK、(G)MSK 和 BPSK 调试模式。产品配置按键、指示灯、调试器、OLED显示屏、通用传感器接口、EEPROM 存储器等 MCU 典型外围器件，单 USB Type C 接口供电，支持串口通讯、程序下载和在线调试功能。   
