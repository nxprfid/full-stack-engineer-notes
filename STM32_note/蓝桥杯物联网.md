...
8点55分：开启录屏软件，比赛全程不得关闭或暂停，并保留录屏文件至少60天。  

9点00分：正式比赛。选手在比赛系统点击“共享屏幕”，开考后下载试题并解压（试题解压密码见系统公告）。
...
有道云笔记：
https://note.youdao.com/s/Z4B9pcLj  
...
void OLED_Proc(void)
{
    case STATE_ADC_IN3:
        ...
        sprintf((char *)oled_buf_Line1, "IN3:%4d", ADC_Read());                         //ADC显示
    }
}
...
void KEY_Proc(void)
{
    if(KEY.KEY_down == ASW1)
    {
        ...
        else if(State == STATE_ADC_IN3) State = STATE_MAIN;
    }
    if(KEY.KEY_down == ASW2)
    {
        if(State == STATE_MAIN) State = STATE_ADC_IN3;
        else if(State == STATE_TIME_NUM) State = STATE_MAIN;
        else if(State == STATE_KEY) State = STATE_TIME_NUM;
...
uint16_t ADC_Read(void)
{
    ...
    while(HAL_ADC_PollForConversion(&hadc, 200) != HAL_OK);         //等待转化完成
    return HAL_ADC_GetValue(&hadc);                                 //返回数值
}
```
...
main.c
```c
    HAL_TIM_Base_Start_IT(&htim2);                                      //定时器2启动 并开启中断
```
stm32wlxx_it.c
```c
#include "task.h"
...
{
    if(htim->Instance == TIM2)                                                  //判断是否是来自定时器2
    {
        Time_Num++;                                                             //是则时间增加
    }
}
...
main.c
```c
        HAL_TIM_Base_Start_IT(&htim2);                    //启动定时器2 并开启中断
    HAL_TIM_Base_Start(&htim1);                       //启动定时器1 不开启中断
    HAL_TIM_OC_Start(&htim1, TIM_CHANNEL_4);          //启动比较输出 定时器1通道四
```
### 脉冲输出模块
LED：PA12，PA11   
可调电阻 ：PB3   
脉冲模块输出：PA15 TIM2_CH1   
![alt text](image-29.png)
TIM2中断也要使能
main.c
```c
    HAL_TIM_Base_Start(&htim2);                       //开启定时器2 不开启中断
    HAL_TIM_IC_Start_IT(&htim2, TIM_CHANNEL_1);       //开启输入捕获中断 定时器2通道1
    HAL_TIM_IC_Start_IT(&htim2, TIM_CHANNEL_2);       //开启输入捕获中断 定时器2通道二
```
task.h
```c
#define USE_EX04_PULSE
#ifdef USE_EX04_PULSE
extern uint32_t IC1_Width;
extern uint32_t IC2_Pulse;
#endif
...
uint32_t IC2_Pulse;
    ...
    case STATE_PULSE:
        if(Old_State != STATE_PULSE)
        {
            Old_State = STATE_PULSE;
            OLED_Clear();
        }
        sprintf((char *)oled_buf_Line1, "width:%8d", IC1_Width);
        sprintf((char *)oled_buf_Line2, "Pulse:%8d", IC2_Pulse);
        break;
    ...
    if(KEY.KEY_down == ASW1)
    {
        ...
        else if(State == STATE_KEY) State = STATE_PULSE;
        else if(State == STATE_PULSE) State = STATE_MAIN;
    }
    if(KEY.KEY_down == ASW2)
    {
        if(State == STATE_MAIN) State = STATE_PULSE;
        else if(State == STATE_TIME_NUM) State = STATE_MAIN;
        else if(State == STATE_KEY) State = STATE_TIME_NUM;
        else if(State == STATE_PULSE) State = STATE_KEY;
    }
```
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)                     //定时器更新回调函数
{
    if(htim->Instance == TIM2)                                                  //因为没有开启更新中断 所以不会触发
    {
        Time_Num++;
    }
}
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)                        //定时器输入捕获回调函数
{
    ...
    {
        if(htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1)                           //判断是否来自活动通道1
        {
            IC1_Width = __HAL_TIM_GET_COMPARE(&htim2, TIM_CHANNEL_1);           //是则记录数值
        }
        if(htim->Channel == HAL_TIM_ACTIVE_CHANNEL_2)                           //判断是否来自活动通道2
        {
            IC2_Pulse = __HAL_TIM_GET_COMPARE(&htim2, TIM_CHANNEL_2);           //是则记录数值
        }
    }
}
```
