...
>- 单个比特的周期时间应至少为 1.25 微秒【要求 1】（数据手册中的数值），且最多为 9 微秒【要求 2】（复位信号的触发时间）。  
...
其中SPI_SCLK、SPI_MOSI、SPI_CS 配置为高速输出，SPI_MISO 配置为上拉输入。  
...
## 使用 SPI：选择位数
SPI 是迄今为止输出相同长度脉冲的最简单方法。我们需要为每个 WS1812 位输出几个 SPI 位（即高脉冲和低脉冲）。WS2812/WS2812B 的时序要求决定了 SPI 外设的速度。它还取决于我们用于创建单个 WS1812 位的 SPI 位（脉冲）的数量。我们会发现更多的 bits 允许更广泛的速度范围。  

### 选项 1：使用 3 个 SPI 位/脉冲  
这是我们可以使用的最小比特数。  
...
1. 3 个脉冲> 1.25 μs → 1 个脉冲 > 417 ns  
1. 3 个脉冲 < 9 μs → 1 个脉冲 < 3000 ns  
1. 1 个脉冲 > 62.5 ns  
1. 1 个脉冲 < 500 ns  
1. 2 个脉冲 > 625 ns → 1 个脉冲 > 413 ns  
导致：  

417 ns <脉冲 < 500 ns → 2.00 Mb/s < SPI 比特率 < 2.39 Mb/s  
### 选项 2：使用 4 个 SPI 位/脉冲  
- “0” 将被编码为 1000  
- “1” 将被编码为 1110  

时序要求：  

1. 4 个脉冲 > 1.25 μs → 1 个脉冲 > 313 ns  
1. 4 个脉冲 < 9 μs → 1 个脉冲 < 2250 ns  
1. 1 个脉冲 > 62.5 ns  
1. 1 个脉冲 < 500 ns  
1. 3 个脉冲 > 625 ns → 1 个脉冲 > 208 ns  
导致：313 ns <脉冲 < 500 → 2.00 Mb/s < SPI 比特率 < 3.19 Mb/s  

### 选项 3：使用 8 个 SPI 位/脉冲  
- “0” 将被编码为 10000000  
- “1” 将被编码为 11111100  
时序要求：  

1. 8 个脉冲 > 1.25 μs → 1. 1 个脉冲 > 157 ns  
1. 8 个脉冲 < 9 μs → 1 个脉冲 < 1125 ns  
1. 1 个脉冲 > 62.5 ns  
1. 1 个脉冲 < 500 ns  
1. 6 个脉冲 > 625 ns → 1 个脉冲 > 105 ns  
导致：157 ns <脉冲 < 500 ns → 2.00 Mb/s < SPI 比特率 < 6.36 Mb/s  

[摘录此网站](https://www.newinnovations.nl/post/controlling-ws2812-and-ws2812b-using-only-stm32-spi/)  
