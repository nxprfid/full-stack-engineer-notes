...
自动清理不再需要的依赖项：  
`sudo apt autoremove`。这个命令会删除那些由于安装 ROS 而安装但现在不再需要的其他软件包。
...
# 换源
npn换源：由于网络延迟问题，建议将 npm 源切换为国内的镜像源，例如淘宝源，以加快安装速度：
...
没有接I2C触摸屏， 系统开机后检测不到会重启；如果触摸坏了， 可以在ido-pi-oh3566-mipi-v1.dts⾥ 关闭2c1
&i2c1 {
status = "disabled"; };
