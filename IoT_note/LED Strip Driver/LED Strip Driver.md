...
## AB线差分写码短包无占说明:GS8523/GS8524/GS8526
### 一、完整写码数据格式
**基础DMX512帧格式**：IDLE（空闲高电平0us~1s）→ BREAK（复位低电平≥88us）→ MAB（复位后标记高电平8us~1s）→ START CODE（起始码9bits低电平）→ [START BIT（1bit低电平）+ SLOT通道数据（8bits，低位在前）+ STOP BIT（2bits高电平）]×N → IDLE
**写码配置包完整格式**：连续4帧3通道DMX512数据（帧间空闲<50ms，可以统一为5ms）→ 间隔5ms → 100us低电平复位信号 → 等待指定时长生效
...
- 关键标识：BREAK=≥88us低电平复位信号；MAB=8us~1s高电平标记；配置包帧间空闲必须<50ms；第四帧后必须间隔5ms再发100us复位
...
### 三、写地址核心数据定义
按3种编址模式，分别定义4帧配置包的核心数据（以3通道为例，多通道可按规则扩展）：

#### 1. 正常写入DMX地址（需2轮配置包）
##### 第一轮：地址配置包
| 帧序号 | 3通道数据内容（十六进制） | 核心说明 |
| :---: | :---: | :---: |
| 第1帧 | 0x00, 0x00, 0x00 | 固定全0前导帧，不可修改 |
| 第2帧 | 0x00, 0x00, 0x00 | 固定全0前导帧，不可修改 |
| 第3帧 | 0x0E, DATA2, DATA3 | 地址配置命令帧1，0x0E为固定命令码，DATA2=通道模式+地址低4位，DATA3=地址高6位 |
| 第4帧 | 0x0D, DATA2, DATA3 | 地址配置命令帧2，与第3帧DATA2、DATA3完全一致，仅命令码为0x0D |
- 帧后操作：第四帧发送完成→间隔5ms→发送100us复位→等待1s
...
| 第1帧 | 0x00, 0x00, 0x00 | 固定全0前导帧，不可修改 |
| 第2帧 | 0x00, 0x00, 0x00 | 固定全0前导帧，不可修改 |
| 第3帧 | 0x26, DATA2, DATA3 | 地址确认命令帧1，0x26为固定命令码，DATA2、DATA3与第一轮完全一致 |
| 第4帧 | 0x25, DATA2, DATA3 | 地址确认命令帧2，与第3帧DATA2、DATA3完全一致，仅命令码为0x25 |
- 帧后操作：第四帧发送完成→间隔5ms→发送100us复位→等待2s，写码完成
...
| 第1帧 | 0x00, 0x00, 0x00 | 固定全0前导帧，不可修改 |
| 第2帧 | 0x00, 0x00, 0x00 | 固定全0前导帧，不可修改 |
| 第3帧 | 0x46, DATA2, DATA3 | 统一地址命令帧1，0x46为固定命令码，DATA2=通道模式+地址低4位，DATA3=地址高6位 |
| 第4帧 | 0x45, DATA2, DATA3 | 统一地址命令帧2，与第3帧DATA2、DATA3完全一致，仅命令码为0x45 |
- 帧后操作：第四帧发送完成→间隔5ms→发送100us复位→等待2s，整串芯片统一地址写入完成
...
| 第1帧 | 0x00, 0x00, 0x00 | 固定全0前导帧，不可修改 |
| 第2帧 | 0x00, 0x00, 0x00 | 固定全0前导帧，不可修改 |
| 第3帧 | 0x06, 0x08, 0x00 | 地址清零命令帧1，0x06为固定命令码，配置值固定为0x08、0x00 |
| 第4帧 | 0x05, 0x08, 0x00 | 地址清零命令帧2，与第3帧配置值完全一致，仅命令码为0x05 |
- 帧后操作：第四帧发送完成→间隔5ms→发送100us复位→等待2s，芯片地址清零完成

### 四、完整写码发送步骤

#### 模式1：正常写入DMX地址完整步骤
1. 发送第一轮地址配置包：连续发送4帧DMX512数据，帧间空闲时间<50ms；
2. 第四帧发送完成后，间隔5ms，发送100us低电平复位信号；
3. 等待1s，确保芯片暂存地址配置；
4. 发送第二轮地址确认包：连续发送4帧DMX512数据，配置值与第一轮完全一致，帧间空闲时间<50ms；
5. 第四帧发送完成后，间隔5ms，发送100us低电平复位信号；
6. 等待2s，地址写入芯片存储，操作完成。

#### 模式2：统一地址写入完整步骤
1. 勾选「统一地址」选项，设置目标起始地址与通道数；
2. 发送统一地址配置包：连续发送4帧DMX512数据，帧间空闲时间<50ms；
3. 第四帧发送完成后，间隔5ms，发送100us低电平复位信号；
4. 等待2s，单颗/整串芯片统一地址写入完成。

#### 模式3：地址清零完整步骤
1. 勾选「地址清零」选项，设置对应通道数；
2. 发送地址清零配置包：连续发送4帧DMX512数据，帧间空闲时间<50ms；
3. 第四帧发送完成后，间隔5ms，发送100us低电平复位信号；
4. 等待2s，芯片地址清零完成。
...
3. 帧格式约束：
   - 所有配置包前2帧必须为全0数据，不可修改；
   - 第3、4帧的配置值必须完全一致，仅命令码不同，否则芯片校验不通过；
   - 帧间空闲时间必须<50ms，否则芯片丢失前导帧，写码失败；
4. 时序约束：复位信号必须在第四帧发送完成后间隔5ms再发送，时长固定100us，提前/延后/时长错误都会导致配置不生效；
5. 等待时长约束：必须严格遵守复位后的等待时长，不可提前发送下一轮数据，否则芯片无法完成配置存储；
6. 端口约束：必须正确选择编址输出端口，未选中端口无法发送写码数据。
