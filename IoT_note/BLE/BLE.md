...
- 🌐 蓝牙Mesh协议建立在BLE的物理层和链路层之上，实现了多对多的通信模式，使得网络中的设备可以广泛部署并相互通信，消息可通过中继设备实现大范围传输。  
- 📢 蓝牙Mesh网络内部节点间的通信主要采用广播（Advertising）方式，设备以接近100%的占空比扫描广播信道，确保信息不丢失，并优化了消息发送延迟。  
- 🤝 蓝牙Mesh采用发布/订阅消息系统，通过引入“组”的概念实现设备间的松耦合，发布者将消息发送到组地址，订阅者接收来自组地址的消息，极大简化了设备管理和替换。  
- 🔄 蓝牙Mesh采用管理型泛洪方式进行网络信息传输，所有具备中继功能的设备都会转发消息，并通过消息缓存和TTL（Time to Live）机制优化泛洪，减少冗余信息。  
- 💡 蓝牙Mesh节点可配置中继、代理、友谊和低功耗等特性，其中代理特性允许非Mesh的BLE设备（如手机）通过GATT承载与Mesh网络通信，友谊特性则帮助低功耗设备缓存消息以节省电量。  
- 🧩 蓝牙Mesh定义了模型（Models）作为基本功能单位，包含状态、消息和行为，并基于服务器/客户端架构，提供了通用开关、灯光亮度等标准化模型，也支持厂商自定义模型。  
- 🔒 蓝牙Mesh强制实施多层安全机制，包括设备加网时的256位椭圆曲线加密和带外数据认证，所有Mesh消息都经过128位AES-CCM加密和认证，并通过序列号和密钥刷新防止重发攻击和垃圾桶攻击。  
## 蓝牙 Mesh 1.1 
