...
```
### 主配网设备
`mesh_beacon_save_provisioning_data`函数保存配网信息到Flash
...
### 
在bleMesh.c里面引入头文件`#include "mesh_beacon_provisioning.h" // 自定义配网Beacon头文件`和`#include "mesh_beacon_demo.h"`  
定义事件：
```c
...
```
```c
void mesh_beacon_demo_init(void)
{
    DEMO_LOG("Mesh Beacon Demo initialized\r\n");
    DEMO_LOG("Press P15 key to send provisioning beacon\r\n");
    DEMO_LOG("Device will automatically provision when receiving beacon\r\n");
}
```
...
mesh_beacon_handle_received_beacon函数原型
```c
/**
